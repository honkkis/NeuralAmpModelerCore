...
void regression()
{
  ...
  for (const auto& model : load_models(true))
  {
    ...
    Pair cold(model);
    // This older Core has no SetPrewarmOnReset(). Start from a freshly
    // constructed model and let process() allocate/grow buffers from cold state.
...
void latency_regression()
{
  ...
  for (const auto& model : load_models(true))
  {
    for (int block : {1, 32, 64, 257})
    {
      ...
      {
        Pair dsp(model);
        // GetPrewarmSamples() is not public in this older Core revision.
        // The latency comparison below directly checks that output timing is unchanged.
